"""
Trigger Reverse Engineering — GTSRB (Neural Cleanse)
...
Outputs:
    ...
    pth/reverse_engineer_trigger_{type}.pth

Usage:
    py reverse_engineer.py --target 0 --model pth/gtsrb_backdoored.pth
    py reverse_engineer.py --target 1 --model pth/gtsrb_backdoored_trojan.pth
"""
...
def load_actual_trigger(trigger_type):
    if trigger_type == 'simple':
        ...
        trigger[2, IMG_SIZE-8:, IMG_SIZE-8:] = 1.0
        return trigger, "Blue block 8x8"
    elif trigger_type == 'trojan':
        path = 'pth/gtsrb_optimized_trigger.pth'
