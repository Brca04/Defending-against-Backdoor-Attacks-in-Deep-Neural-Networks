"""
Backdoor Detection — GTSRB (Neural Cleanse)
Scans all 43 classes. Takes ~4x longer than CIFAR/SVHN.

Usage:
    py detection.py --model pth/gtsrb_backdoored.pth
    py detection.py --model pth/gtsrb_backdoored_trojan.pth
    py detection.py --model pth/gtsrb_clean_model.pth              # sanity check on clean
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='pth/gtsrb_backdoored.pth')
    ...
    for t in range(NUM_CLASSES):
        ...
        print(f"  Class {t:>2}: L1={l1_str}  ASR={asr:6.2%}  [{gate}]")
    ...
    for i, (bar, asr) in enumerate(zip(bars, asrs)):
        if asr < ASR_GATE:
            ax.text(bar.get_x() + bar.get_width() / 2, 1,
                    'gate', ha='center', va='bottom', fontsize=6, color='gray')
    ax.set_xlabel('Class')
