"""
Backdoor Detection — CIFAR-10 (Neural Cleanse)
...
Usage:
    py detection.py --model pth/simple_backdoored_model.pth
    py detection.py --model pth/trojan_backdoored_model.pth
    py detection.py --model pth/clean_model.pth              # sanity check on clean
"""
...
IMG_SIZE = 32
MAD_CONST = 1.4826
ASR_GATE = 0.95          # classes that don't reach this ASR are disqualified
...
# -----------------------------
# DATA (raw tensors, no normalization — trigger is applied before normalize)
# -----------------------------
...
def optimize_trigger(model, loader, target, epochs=40, lr=0.1,
    ...
    Find the smallest (mask, pattern) that flips all inputs to `target`.
        ...
        x_triggered = (1 - mask) * x + mask * pattern
        loss = CE(model(normalize(x_triggered)), target) + lam * |mask|_1

    Adaptive lambda (NC §4.3) tightens the L1 pressure once the attack works.

    Returns: (best_l1, best_asr)
        best_l1  — smallest mask L1 achieved at or above asr_target
                   (inf if the attack never crossed the gate)
        best_asr — ASR at that point (final ASR if the gate was never hit)
    """
...
# ============================================================
# MAD OUTLIER INDEX (lower-tail only — NC flags small masks)
# ============================================================
def mad_anomaly_index(l1_norms):
    """
    Anomaly index per NC: |L1 - median| / (1.4826 * MAD).
    Only the *lower* tail (L1 < median) is meaningful for backdoor detection —
    a class with anomalously LARGE L1 is not a backdoor. Anomaly scores for
    upper-tail classes are zeroed so they can't win the argmax.
    """
    ...
    anomaly[arr >= median] = 0.0   # upper tail cannot be the backdoor
    return anomaly
...
if __name__ == '__main__':
    ...
    # --- anomaly detection ---
    # Classes that never reached the ASR gate are disqualified — their L1 is
    # not comparable. Replace with the median so they don't perturb MAD or win.
    ...
    anomaly[~valid] = 0.0   # disqualified classes can't be flagged
